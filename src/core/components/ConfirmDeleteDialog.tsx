    ...
    <Dialog.Root open={open} onOpenChange={(isOpen) => { if (!isOpen) onClose() }}>
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-xl shadow-xl p-8 flex flex-col gap-4">
          <Dialog.Title className="text-xl font-semibold text-brand-ink">
            {title}
          </Dialog.Title>
          ...
          <div className="flex justify-end gap-3 mt-2">
            <Dialog.Close render={
              <button
                className="h-10 px-4 py-2 border border-gray-300 rounded-md bg-white hover:bg-gray-50 text-base font-medium transition-colors text-brand-ink"
              >
                Annulla
              </button>
            } />
            <button
              className="h-10 px-4 py-2 rounded-md text-white text-base font-medium transition-opacity hover:opacity-80 bg-brand"
              onClick={onConfirm}
            >
              Elimina
            </button>
